      ...
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "0 16px 40px" }}>
        ...
        {transactions.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {transactions.map((tx) => {
              const typeInfo = txTypeLabels[tx.type] || txTypeLabels[tx.transactionType] || { label: tx.type || tx.transactionType, icon: "📄", color: "#6b7280" };
              const amount = tx.amount || 0;
              const isIncome = ["TopUp", "Refund", "Earning"].includes(tx.type || tx.transactionType);
